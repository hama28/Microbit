...
#BTLE_NAME_MICROBIT = 'BBC micro:bit [zozog]'
#BTLE_NAME_MICROBIT = 'DfuTarg'
...
class Folo_Control (Scene):
	def setup(self):
		...
		self.background_color = '#a0d8e0'
		
		label_status = LabelNode('Status', position=(280,700), parent=self, color='black')
		
		self.follow_button = SpriteNode('iob:arrow_up_a_256', position=(285,510))
		self.add_child(self.follow_button)
		
		self.back_button   = SpriteNode('iob:arrow_down_a_256', position=(285,170))
		self.add_child(self.back_button)
		
		self.right_button  = SpriteNode('iob:arrow_right_a_256', position=(450,350))
		self.add_child(self.right_button)
		
		self.left_button   = SpriteNode('iob:arrow_left_a_256', position=(120,350))
		self.add_child(self.left_button)
		
		self.connect_button = SpriteNode('iob:bluetooth_32', position=(90,700))
	...
	def touch_ended(self, touch):
		...
		if self.current_button == self.connect_button and touch_loc in self.connect_button.frame:
			if is_connected :
				...
				is_connected = False
				delegate = None
				label_status.text = '接続解除'
			else :
				# start conenction
				label_status.text = 'スキャン中 ...'
